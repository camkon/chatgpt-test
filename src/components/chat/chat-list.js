...
const ChatList = ({chat, loading}) => {
    ...
    const QuestionBox = ({question}) => {
        return(
            <Box sx={{display: 'flex', justifyContent: 'center', boxSizing: 'border-box'}}>
                <Box sx={{display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-start', columnGap: '1.5rem', padding: '1.5rem 0', color: '#fff', width: '50%', wordBreak: 'break-word', boxSizing: 'border-box'}}>
    ...
    const AnswerBox = ({data}) => {
        return(
            <Box sx={{display: 'flex', justifyContent: 'center', boxSizing: 'border-box', border: '1px solid #2c2e35', background: '#40414f'}}>
                <Box sx={{display: 'flex', alignItems: 'flex-start', justifyContent: 'flex-start', columnGap: '1.5rem', padding: '1.5rem 0', color: '#c9ccd3', width: '50%', wordBreak: 'break-word', boxSizing: 'border-box'}}>
                    <Avatar sx={{background: '#19c37d', borderRadius: '0.25rem', height: '2rem', width: '2rem', fontSize: '0.9rem'}}>C</Avatar>
                    {data?.error == undefined
                        ? data?.answer !== undefined 
                            ? <p style={{margin: '0.5rem 0 0 0'}}>{data?.answer}</p>
    ...
    return (
        <Box sx={{overflowY: 'scroll', height: 'calc(100vh - 7rem)', paddingBottom: '12rem', scrollBehavior: 'smooth'}} ref={heightRef}>
            {chat?.map(item => [
